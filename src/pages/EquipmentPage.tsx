...
export default function EquipmentPage() {
  ...
  const labels = {
    title: isRTL ? 'معدات الإصلاح' : 'Équipements de Réparation',
    subtitle: isRTL ? 'أدوات ومعدات إصلاح الهواتف المحمولة' : 'Outils et équipements de réparation mobile',
    search: isRTL ? 'ابحث عن معدات...' : 'Rechercher un équipement...',
    allCities: isRTL ? 'جميع المدن' : 'Toutes les villes',
    allConditions: isRTL ? 'جميع الحالات' : 'Toutes conditions',
    new: isRTL ? 'جديد' : 'Neuf',
    used: isRTL ? 'مستعمل' : 'Occasion',
    noResults: isRTL ? 'لا توجد معدات متاحة' : 'Aucun équipement disponible',
  ...
  useEffect(() => {
    const loadItems = async () => {
      setLoading(true);
      const filters: any = { itemType: 'equipment' };
  ...
  return (
    <div className="min-h-screen bg-gray-50" dir={isRTL ? 'rtl' : 'ltr'}>
      <Navigation />
      <BannerSlot page="equipment" slot="top" />
                  ...
                  <Link key={item.id} to={`/items/${item.slug}`}>
                    <Card className="overflow-hidden hover:shadow-lg transition-shadow h-full group">
                      ...
                      <CardContent className="p-4">
                        ...
                        <p className={cn('font-bold text-lg text-purple-600', isRTL && 'text-right')}>{formatPrice(item.price, item.price_text)}</p>
      ...
      <BannerSlot page="equipment" slot="bottom" />
