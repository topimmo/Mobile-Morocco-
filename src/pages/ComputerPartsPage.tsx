...
import { getComputerParts, ComputerPartWithRelations, COMPUTER_PART_CATEGORIES } from '@/lib/supabase/computers';
...
export default function ComputerPartsPage() {
  ...
  const [items, setItems] = useState<ComputerPartWithRelations[]>([]);
  ...
  const [partCategory, setPartCategory] = useState(searchParams.get('category') || '');
  ...
  const labels = {
    title: isRTL ? 'قطع الحاسوب' : 'Pièces Informatiques',
    subtitle: isRTL ? 'تصفح جميع قطع الحاسوب المتاحة' : 'Parcourez toutes les pièces informatiques disponibles',
    search: isRTL ? 'ابحث عن قطعة...' : 'Rechercher une pièce...',
    ...
    noResults: isRTL ? 'لا توجد قطع متاحة' : 'Aucune pièce disponible',
    ...
    publishPart: isRTL ? 'نشر قطعة' : 'Publier une pièce',
  };
  ...
  useEffect(() => {
    const loadItems = async () => {
      ...
      if (partCategory) filters.partCategory = partCategory;

      // Generate cache key
      const cacheKey = SimpleCache.generateKey('computer-parts', { ...filters, page, perPage: 20 });
      
      // Check cache first
      const cached = apiCache.get<{ data: ComputerPartWithRelations[]; count: number }>(cacheKey);
      if (cached && page === 1) {
      ...
      try {
        const { data, count } = await getComputerParts(filters, { page, perPage: 20 });
        ...
        console.error('Error loading computer parts:', error);
      } finally {
        setLoading(false);
        loadingRef.current = false;
      }
    ...
    loadItems();
  }, [debouncedKeyword, cityId, partCategory, page]);
  ...
  const handleSearch = () => {
    ...
    if (partCategory) params.set('category', partCategory);
  ...
  const getItemTitle = (item: ComputerPartWithRelations) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <BannerSlot page="computer-parts" slot="top" />
        ...
        <div className="max-w-7xl mx-auto">
          <div className={cn('flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 mb-4', isRTL && 'sm:flex-row-reverse')}>
            ...
            <div className={cn('sm:ml-auto w-full sm:w-auto', isRTL && 'sm:mr-auto sm:ml-0')}>
              <Link to="/publish-computer-part" className="w-full sm:w-auto">
          ...
          <div className="max-w-5xl mx-auto">
            <div className={cn('grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-[1fr_auto_auto_auto_auto] gap-3', isRTL && 'lg:grid-flow-dense')}>
              ...
              <Select value={partCategory || 'all'} onValueChange={(v) => { setPartCategory(v === 'all' ? '' : v); setPage(1); }}>
                <SelectTrigger className="w-full">
                  ...
                  <Link key={item.id} to={`/items/${item.slug}`}>
                    <Card className="overflow-hidden hover:shadow-lg transition-shadow h-full group">
                      <div className="relative h-48 bg-gray-200">
                        ...
                        {item.computer_details?.part_category && (
                          <Badge 
                            className={cn('absolute top-2', isRTL ? 'left-2' : 'right-2', 'bg-purple-600')}
                          >
                            {item.computer_details.part_category}
                          </Badge>
                        )}
                      ...
                      <CardContent className="p-4">
                        ...
                        <p className={cn('font-bold text-lg text-purple-600', isRTL && 'text-right')}>
                        ...
                        {item.computer_details && (
                          <div className={cn('flex flex-wrap gap-1 mt-2', isRTL && 'flex-row-reverse')}>
                            {item.computer_details.capacity && (
                              <Badge variant="outline" className="text-xs">{item.computer_details.capacity}</Badge>
                            )}
                            {item.computer_details.part_type && (
                              <Badge variant="outline" className="text-xs">{item.computer_details.part_type}</Badge>
                            )}
                            {item.computer_details.stock_quantity && (
                              <Badge variant="outline" className="text-xs">
                                {isRTL ? `${item.computer_details.stock_quantity} متوفر` : `${item.computer_details.stock_quantity} en stock`}
                              </Badge>
                            )}
                          </div>
                        )}
      ...
      <BannerSlot page="computer-parts" slot="bottom" />
