      ...
      <PageLayout dir={isRTL ? 'rtl' : 'ltr'}>
        ...
        <PageMain className="bg-background">
          <Container className="py-16 text-center">
            <Smartphone className="h-24 w-24 text-muted-foreground/30 mx-auto mb-6" />
            <h1 className="text-2xl font-bold text-foreground mb-2">{labels.notFound}</h1>
            <p className="text-muted-foreground mb-6">{labels.notFoundHint}</p>
            <Link to="/phones">
        ...
        <Container className="py-8 md:py-12">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12">
                ...
                <div className="relative bg-muted/30">
                  {images.length > 0 ? (
                    ...
                    <div className="w-full h-96 flex items-center justify-center">
                      <ItemIcon className="h-24 w-24 text-muted-foreground/30" />
                    </div>
                  )}
                  ...
                  {images.length > 1 && (
                    <>
                      <button
                        ...
                        className={cn(
                          'absolute top-1/2 -translate-y-1/2 bg-black/50 text-white p-2 rounded-full hover:bg-black/70 transition-colors',
                  ...
                  <Badge 
                    className={cn(
                      'absolute top-4',
                      isRTL ? 'left-4' : 'right-4',
                      item.condition === 'new' ? 'bg-green-600 hover:bg-green-600' : 'bg-orange-500 hover:bg-orange-500'
                    )}
                  >
            ...
            <div className="space-y-6">
              <div>
                <h1 className={cn('text-3xl md:text-4xl font-bold tracking-tight mb-3', isRTL && 'text-right')}>
                  {getItemTitle(item, language)}
                </h1>

                <p className={cn('text-3xl md:text-4xl font-bold text-primary', isRTL && 'text-right')}>
              ...
              {(item.brand || item.model) && (
                <div className={cn('flex gap-6', isRTL && 'flex-row-reverse')}>
                  {item.brand && (
                    <div>
                      <span className="text-muted-foreground text-sm block mb-1">{labels.brand}</span>
                      <span className="font-medium">{item.brand}</span>
                    </div>
                  )}
                  {item.model && (
                    <div>
                      <span className="text-muted-foreground text-sm block mb-1">{labels.model}</span>
                    ...
                    <div className={cn('flex items-center justify-between', isRTL && 'flex-row-reverse')}>
                      <div className={cn('flex items-center gap-3', isRTL && 'flex-row-reverse')}>
                        ...
                        <div>
                          ...
                          <div className={cn('flex items-center gap-2 text-sm text-muted-foreground mt-1', isRTL && 'flex-row-reverse')}>
              ...
              {(item.description_fr || item.description_ar) && (
                <div>
                  ...
                  <p className={cn('text-muted-foreground whitespace-pre-wrap leading-relaxed', isRTL && 'text-right')}>
                        ...
                        <div className="relative h-48 bg-muted/30">
                          {similar.images && similar.images[0] ? (
                            ...
                            <div className="w-full h-full flex items-center justify-center">
                              <SimilarIcon className="h-16 w-16 text-muted-foreground/30" />
                            </div>
                          )}
                          <Badge className={cn('absolute top-2', isRTL ? 'left-2' : 'right-2', similar.condition === 'new' ? 'bg-green-600 hover:bg-green-600' : 'bg-orange-500 hover:bg-orange-500')}>
