...
export default function PhonesPage() {
  ...
  const labels = {
    title: isRTL ? 'الهواتف' : 'Téléphones',
    ...
    noResults: isRTL ? 'لا توجد هواتف متاحة' : 'Aucun téléphone disponible',
    ...
    publishPhone: isRTL ? 'نشر تلفوني' : 'Publier mon téléphone',
  ...
  useEffect(() => {
    const loadItems = async () => {
      ...
      // Generate cache key
      const cacheKey = SimpleCache.generateKey('phones', { ...filters, page, limit: 20 });
      
      // Check cache first
      const cached = apiCache.get<{ data: ItemWithRelations[]; count: number }>(cacheKey);
      if (cached && page === 1) {
      ...
      try {
        const { data, count } = await getItems(filters, { page, limit: 20 });
  ...
  return (
    <div className="min-h-screen bg-gray-50" dir={isRTL ? 'rtl' : 'ltr'}>
      ...
      <BannerSlot page="phones" slot="bottom" />
